...
// https://www.pexels.com/photo/interior-of-modern-kitchen-with-wooden-furniture-7031617/
...
if (source.split(oldCredit).length !== 2) {
  throw new Error('Crédito da antiga fotografia não localizado exatamente uma vez; publicação interrompida.');
}
source = source.replace('<br>' + oldCredit, '');
...
<img src="https://images.pexels.com/photos/7031617/pexels-photo-7031617.jpeg?auto=compress&cs=tinysrgb&w=960" alt="Cozinha contemporânea com armários em madeira" loading="lazy" decoding="async">
</figure>`;
source = source.replace(oldPanel, newPanel);

// Remover avisos visuais das imagens provisórias sem declarar que são projetos próprios.
const notices = [
  '<small class="photo-legend">Fotografia de referência. Não representa um projeto executado pela Vorela.</small>',
  '<p class="disclaimer">Estas categorias são inspirações de ambientes, não fotografias de trabalhos realizados pela Vorela. O portfólio será formado somente com projetos e imagens autorizados.</p>'
];
for (const notice of notices) {
  if (source.split(notice).length !== 2) {
    throw new Error('Aviso visual esperado não localizado exatamente uma vez; publicação interrompida.');
  }
  source = source.replace(notice, '');
}
